...
import { 
  ...
  getChattableCandidatesAction, 
  syncCandidateToStreamAction 
} from "@/app/actions/stream-actions"
import { toast } from "sonner"
import { 
  Loader2, Search, SendHorizontal, AlertCircle, 
  User, ArrowLeft, Info, Mail, Trash2, 
  MessageSquarePlus, CheckCheck, Phone, MapPin, FileText
} from "lucide-react"
...
const CandidateProfileSheet = ({ 
  ...
  const handleDeleteConversation = async () => {
    if (!channel) return;
    const confirm = window.confirm("Delete this conversation permanently?");
    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent className="w-[100%] sm:w-[400px] overflow-y-auto bg-white border-l shadow-2xl z-[100] flex flex-col h-full text-slate-900 px-6 pb-8">
        <SheetHeader className="pb-6 border-b border-slate-50">
          <SheetTitle className="text-slate-900 text-xl font-bold">Candidate Profile</SheetTitle>
        </SheetHeader>
        ...
        <div className="flex-1 py-8">
          <div className="flex flex-col items-center text-center space-y-5">
            ...
            <div>
              ...
              <span className="bg-blue-50 text-[#1C3FA4] text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wide inline-block mt-2">
                Applicant
              </span>
            </div>
          </div>
          ...
          <div className="mt-10 space-y-8">
            <div className="space-y-4">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest px-1">Candidate Details</h3>
                ...
                {resume && (
                  <div className="pt-4 mt-2 border-t border-slate-200/50">
                    <a 
                      ...
                      className="flex items-center justify-center gap-2 w-full p-3 bg-white border border-slate-200 rounded-xl hover:border-[#1C3FA4] hover:text-[#1C3FA4] transition-all group shadow-sm"
                    >
                      <FileText className="w-4 h-4 text-slate-400 group-hover:text-[#1C3FA4]" />
                      <span className="text-sm font-bold text-slate-700 group-hover:text-[#1C3FA4]">View Resume</span>
                    </a>
                  </div>
                )}
...
export default function ReferrerMessagesPage() {
  ...
  React.useEffect(() => {
    ...
    const init = async () => {
      try {
        const [tokenData, candList] = await Promise.all([
          getStreamTokenAction(),
          getChattableCandidatesAction()
        ]);
        ...
        await chatClient.connectUser(
          { 
            id: tokenData.userId, 
            name: tokenData.userName || "Referrer", 
  ...
  const handleStartChat = async (candidate: Candidate) => {
    ...
    const existingChannel = channels.find(c => 
      Object.values(c.state.members).some(m => m.user_id === candidate.id)
    );
    ...
    try {
      await syncCandidateToStreamAction(candidate.id); 

      const channel = client.channel('messaging', {
        members: [client.userID!, candidate.id],
      });
  ...
  const filteredCandidates = candidates.filter(c => {
    if (client && c.id === client.userID) return false;
        ...
        `}>
          ...
          <ScrollArea className="flex-1 px-3">
            {searchQuery.length > 0 ? (
              <div className="pb-4">
                <p className="text-[11px] font-bold text-[#1C3FA4] uppercase tracking-widest px-4 py-2 mb-2">Candidates Found</p>
                {filteredCandidates.map(c => (
                  ...
                  >
                     <Avatar className="w-10 h-10 border border-slate-100 bg-white">
                        ...
                        <AvatarFallback className="bg-blue-100 text-[#1C3FA4] font-bold">{c.full_name[0]}</AvatarFallback>
                      </Avatar>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-slate-900">{c.full_name}</p>
                        <p className="text-xs text-slate-500 truncate">{c.email || "Candidate"}</p>
                      </div>
              ...
              <div className="pb-4 space-y-1">
                {channels.length === 0 ? (
                  <div className="flex flex-col items-center justify-center text-center p-8 mt-10">
                    ...
                    <p className="text-slate-500 text-sm mt-1 max-w-[200px]">Search for a candidate to start messaging.</p>
                  </div>
                ) : (
